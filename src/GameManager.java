...
public class GameManager extends Applet implements Runnable {
	...
	Image playernamesImg[]=new Image[5];//5 player designed name banners
	   ...
	   for (int i=0; i<3; i++)
	    {
	       ...
	       attackImages[i] = getImage(getCodeBase(),"attack" +(i+1)+ ".gif");
		...
		public void paint(Graphics g)
		{
		   if((playing)||(screen==GAMEPLAY))
		    ...
		    else if (screen == INTRO) 
		    {
			   ...
			   offscreen.drawString(ufoLandedString+numLanded,width - 113,27);
		    ...
		    else if (screen == GAME_OVER) 
		     {
			   ...
			   offscreen.setFont(bigFont);
			   offscreen.setColor(Color.red);
			   offscreen.drawString(gameOverString,(width-stringWidth)/2,height/2);
			   offscreen.setFont(mediumFont);
			   offscreen.setColor(Color.green);
			   offscreen.drawString(clickString,(width-stringWidth-17)/2,height*11/12);
			   g.drawImage(image,0,0,this);
			 }
          }
